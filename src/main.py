...
"""
    ...
    Does a run in batches
    Writes the results to separate files for each type
"""
def run_net2(factory: NetworkController, request_monitor: RequestMonitor, filename, n_type="all"):
    ...
    for i in range(1): #Amount of RUNs
...
filename = "13.12.Battistoni"
#run_net2(network_controller,request_monitor, n_type="all", filename=filename)
bar_grapher.graph_completion_time(filename, filename)
bar_grapher.graph_processing_time(filename, filename)